...
required_packages=("openssl" "zip" "curl" "jq" "build-essential" "make")
...
function usage() { echo "Usage: $0 [-s | --subnet <16|32|48|64|80|96|112> proxy subnet (default 64)] 
                          ...
                          [-d | --disable-inet6-ifaces-check <bool> disable /etc/network/interfaces configuration check & exit when error
                                use only if configuration handled by cloud-init or something like this (for example, on Google Cloud servers)]                                                      
...
# Global network interface name - для Google Cloud это обычно ens4
interface_name="ens4"
...
function get_backconnect_ipv4() {
  ...
  (maybe_ipv4=$(curl -s https://ipinfo.io/ip)) &> /dev/null
  if is_valid_ip $maybe_ipv4; then echo $maybe_ipv4; return; fi;
...
function check_ipv6() {
  ...
  # Для Google Cloud отключаем проверку конфигурации интерфейсов
...
function install_requred_packages() {
  ...
  requred_packages=("make" "g++" "wget" "curl" "cron" "build-essential");
...
function configure_ipv6() {
  ...
  for option in ${required_options[@]}; do
    ...
    if ! grep -q "^$full_option" /etc/sysctl.conf; then echo $full_option >> /etc/sysctl.conf; fi;
  done;
...
# Выполняем первоначальные настройки для Google Cloud
...
# Отключаем firewall (Google Cloud использует свои правила брандмауэра)
...
upload_response=$(curl -s -F "file=@$proxy_dir/proxy.zip" https://file.io)
